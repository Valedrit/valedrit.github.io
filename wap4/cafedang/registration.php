...
{% set mem = func.get('member')|trim|split('@') %}{% set get_token %}{% for a in mem %}{% set udata = get_data('user_'~a|trim)|last.data|json_decode %}{% if get_data_count('user_'~a|trim)!='0' %}"{{udata['token']}}":"{{a|trim}}"{% if loop.last %},{% endif %}{% endif %}{% endfor %}{% endset %}
{% if get_get('act')=='faq' %}
...
{% set token_new %}"{{token}}":"{{user}}"{% endset %}
{{func.add('token',token,user)}}
{{func.add('forum','new_mem',user)}}
{{func.add('forum','time_reg',"now"|date("U")+100)}}
