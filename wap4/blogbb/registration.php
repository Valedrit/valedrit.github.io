...
{% else %}
{% set title = 'Đăng ký tài khoản | BlogBB' %}
{% endif %}
{% if signin %}
...
<b>BlogBB.GQ community</b> là diễn đàn giải trí dành cho các thành viên, tại đây bạn thoải mái giao lưu, kết bạn, nhắn tin và đăng chủ đề, mua bán và nhiều tiện ích khác!
</div>
...
<p><div class="rmenu"><b style="color:red">*</b> Đăng ký tài khoản này chỉ sử dụng được trong khu vực cộng đồng, không thể đăng bài ở blog trang chính. Tên tài khoản khi đăng kí không vượt quá 15 kí tự và tối tiểu phải đặt 3 kí tự, mật khẩu dài tùy ý.</div></p>
<p>Mật khẩu:</p>
<input type="password"  name="pass" value="" required>
<p>Nhập lại mật khẩu:</p>
<input type="password" name="repass" value="" required>
<p>Mã xác nhận: [ <b>{{captcha}}</b> ]</p>
<input type="number" name="captcha" value="" required>
...
{% set save=save_data( "user_"~func.rwurl(user),data|json_encode) %}
{% set save_tb=save_data("notif_"~func.rwurl(user),tb|json_encode ) %}
{{func.add('token',token,user)}}
{{func.add('forum','new_mem',user)}}
{{func.add('forum','time_reg',"now"|date("U")+600)}}
